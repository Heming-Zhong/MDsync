...
异步的问题主要出现在两个方面，一个是访问数据库时，由于大部分数据库提供的接口是异步的，通过回调函数来返回查询结果，这就导致了一部分追求及时性的操作无法使用数据库查询，最后我是换用了一个采用同步接口的`better-sqlite3`来解决的这个问题。另一处困难则是在TCP传输文件时，改变时间戳的时刻很可能发生在文件读写结束前，这会导致同步逻辑暂时失效；这个问题则是将一部分文件操作改为同步，另一部分将改变时间戳调整到文件操作的回调链中执行。
